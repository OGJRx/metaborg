...
/**
 * Generic scheduling flow that supports deep personalization via slot_template
 * and other configuration-driven parameters.
 */
export async function handleGenericAgendadoUpdate(
  ctx: FactoryContext,
...
async function getStepOptions(
  ...
  if (step.type === "time") {
    ...
    return slots
      ...
      .map((s) => ({
        label: config.scheduling.slot_template
          ? config.scheduling.slot_template.replace(`${"${"}time}`, s.hora)
          : s.hora,
        value: s.hora,
      }));
  }
