...
import { TitaniumSession } from "./types";

export class RelationalSessionAdapter implements StorageAdapter<TitaniumSession> {
  ...
  async write(key: string, value: TitaniumSession): Promise<void> {
    ...
    const step_data = JSON.stringify((value as any).step_data || {});
    const paso_actual = (value as any).paso_actual || 0;
    ...
    await this.db
      ...
      .bind(session_id, botId, platform, chatId, paso_actual, step_data, expires_at)
      .run();
  }
